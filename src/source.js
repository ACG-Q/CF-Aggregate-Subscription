...
async function verifyToken(token, secret) {
	...
	const valid = await crypto.subtle.verify('HMAC', key, decodedSignature, encoder.encode(payload));
...
export default {
	async fetch(request, env) {
		...
		if (
			!(token == userToken || token == generatedToken || url.pathname == '/' + userToken || url.pathname.includes('/' + userToken + '?'))
		) {
			// 处理登录请求
			...
			if (subscriptionFormat == 'base64' || token == generatedToken) {
				...
				switch (subscriptionFormat) {
					case 'clash':
						subconverterUrl = `${converterProtocol}://${subscriptionConverterAPI}/sub?target=clash&url=${encodeURIComponent(
							subscriptionConversionURL
						)}&insert=false&config=${encodeURIComponent(
							subscriptionConfigURL
						)}&emoji=true&list=false&tfo=false&scv=true&fdn=false&sort=false&new_name=true`;
						break;
					case 'singbox':
						subconverterUrl = `${converterProtocol}://${subscriptionConverterAPI}/sub?target=singbox&url=${encodeURIComponent(
							subscriptionConversionURL
						)}&insert=false&config=${encodeURIComponent(
							subscriptionConfigURL
						)}&emoji=true&list=false&tfo=false&scv=true&fdn=false&sort=false&new_name=true`;
						break;
					case 'surge':
						subconverterUrl = `${converterProtocol}://${subscriptionConverterAPI}/sub?target=surge&url=${encodeURIComponent(
							subscriptionConversionURL
						)}&insert=false&config=${encodeURIComponent(
							subscriptionConfigURL
						)}&emoji=true&list=false&tfo=false&scv=true&fdn=false&sort=false&new_name=true`;
						break;
					case 'quanx':
						subconverterUrl = `${converterProtocol}://${subscriptionConverterAPI}/sub?target=quanx&url=${encodeURIComponent(
							subscriptionConversionURL
						)}&insert=false&config=${encodeURIComponent(
							subscriptionConfigURL
						)}&emoji=true&list=false&tfo=false&scv=true&fdn=false&sort=false&udp=true`;
						break;
					case 'loon':
						subconverterUrl = `${converterProtocol}://${subscriptionConverterAPI}/sub?target=loon&url=${encodeURIComponent(
							subscriptionConversionURL
						)}&insert=false&config=${encodeURIComponent(
							subscriptionConfigURL
						)}&emoji=true&list=false&tfo=false&scv=true&fdn=false&sort=false`;
						break;
					default:
...
async function sendTelegramNotification(type, ip, add_data = '') {
	...
	if (telegramBotToken !== '' && telegramChatID !== '') {
		...
		let url = `https://api.telegram.org/bot${telegramBotToken}/sendMessage?chat_id=${telegramChatID}&parse_mode=HTML&text=${encodeURIComponent(
			msg
		)}`;
	...
	try {
		...
		for (let i = 0; i < apiUrls.length; i += maxConcurrentRequests) {
			...
			const batchResults = await Promise.allSettled(
				currentBatch.map((apiUrl) =>
					getUrl(request, apiUrl, appendUA, userAgentHeader).then((response) => (response.ok ? response.text() : Promise.reject(response)))
				)
			);
		...
		for (const response of processedResponses) {
			if (response.status === 'fulfilled') {
				...
				if (content.includes('proxies') && content.includes('proxy-groups')) {
					...
					const errorLink = `trojan://CMLiussss@127.0.0.1:8888?security=tls&allowInsecure=1&type=tcp&headerType=none#%E5%BC%82%E5%B8%B8%E8%AE%A2%E9%98%85%20${
						response.apiUrl.split('://')[1].split('/')[0]
					}`;
